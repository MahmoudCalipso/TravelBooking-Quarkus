 ...
 */
@Tag(name = "Website - OAuth", description = "OAuth flows for traveler/supplier/association roles")
        ...
        try {
            ...
            try {
                ...
                ErrorResponse error = new ErrorResponse("INVALID_ROLE",
                        "Invalid role. Supported roles: TRAVELER, SUPPLIER_SUBSCRIBER, ASSOCIATION_MANAGER");
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity(error)
                        .build();
            }

            EnumSet<UserRole> allowedRoles = EnumSet.of(UserRole.TRAVELER, UserRole.SUPPLIER_SUBSCRIBER,
                    UserRole.ASSOCIATION_MANAGER);
            if (!allowedRoles.contains(userRole)) {
                ErrorResponse error = new ErrorResponse("INVALID_ROLE_FOR_OAUTH",
                        "OAuth authentication is only available for TRAVELER, SUPPLIER_SUBSCRIBER, ASSOCIATION_MANAGER roles");
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity(error)
                        .build();
            }
