 ...
 * Media Controller.
 * 
 * Handles media upload and management using Firebase Storage as the single
 * source of truth.
 * Media files are stored in Firebase Storage, and only metadata is stored in
 * the database.
 * 
 * Upload Flow:
...
public class WebMediaController {
    ...
    private static final Logger log = LoggerFactory.getLogger(WebMediaController.class);
     ...
     * the file directly to Firebase Storage. A pending MediaAsset record is
     * created.
     * 
    ...
    public Response requestUploadUrl(@Valid UploadRequestRequest request) {
        ...
        try {
            ...
            // Security Check: Verify that the current user is the owner (for USER type)
            // or has access to the component (for other types like ACCOMMODATION, EVENT
            // etc.)
            // For now, we enforce that users can only upload for themselves if OwnerType is
            // USER.
            // For other types, additional logic would be needed.
            if (request.getOwnerType() == MediaAsset.OwnerType.USER) {
                if (!currentUser.getId().equals(ownerUuid)) {
                    ErrorResponse error = new ErrorResponse("FORBIDDEN",
                            "You can only upload media for your own profile");
                    return Response.status(Response.Status.FORBIDDEN)
                            .entity(error)
                            .build();
                }
            }
            // TODO: Add ownership checks for other OwnerTypes (ACCOMMODATION, EVENT, etc.)
            // This would typically involve checking if the user owns the
            // accommodation/event.
    ...
    public Response confirmUpload(
        ...
        try {
            ...
            if (!canManageMediaAsset(mediaAsset)) {
                ErrorResponse error = new ErrorResponse("FORBIDDEN",
    ...
    private boolean canManageMediaAsset(MediaAsset asset) {
        ...
        if (asset.getOwnerType() == MediaAsset.OwnerType.USER) {
            return currentUser.getId().equals(asset.getOwnerId());
        }

        // TODO: For other types (ACCOMMODATION, EVENT etc.), check if the user is the
        // owner of the entity.
        // For simplicity during RBAC implementation, we allow it if the user is
        // authenticated
        // and add a log warning for production readiness.
        log.warn("Simplified ownership check for MediaAsset type: {}. AssetId: {}, OwnerId: {}",
                asset.getOwnerType(), asset.getId(), asset.getOwnerId());
